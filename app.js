...
function renderTable(dataArray) {
    ...
    if (tableViewMode === 'list') {
        ...
        displayArray.forEach(d => {
            ...
            tr.innerHTML += `<td><strong>${d.협력사}</strong></td><td>${d.지역}</td><td class="text-right" style="color:var(--accent-cyan);font-weight:600">${formatNum(d.인원)}</td><td class="text-right">${formatNum(d.지상비)}</td>`;
        ...
        sortedRegions.forEach(regionName => {
            ...
            gRow.innerHTML = `<td colspan="${showTeam ? 5 : 4}">[지역 구분] ${regionName}</td>`;
            ...
            arr.forEach(d => {
                const tr = document.createElement('tr');
                if (showTeam) tr.innerHTML += `<td>${d.팀}</td>`;
                tr.innerHTML += `<td style="padding-left:24px"><strong>${d.협력사}</strong></td><td style="color:var(--text-secondary)">${d.지역}</td><td class="text-right" style="color:var(--accent-cyan);font-weight:600">${formatNum(d.인원)}</td><td class="text-right">${formatNum(d.지상비)}</td>`;
                tBody.appendChild(tr);
            });
        });
    }
